...
module Commands
  ...
  class HBaseCommand < Command
    attr_accessor :hbase_jar_path, :install_script, :backup_dir, :backup_version

    def initialize(*args)
      super(*args)
      @hbase_jar_path = "/home/hadoop/lib/hbase-0.92.0.jar"
      @install_script = "s3://beta.elasticmapreduce/hbase-beta-0.92.0-1/install-hbase-stage-2"
    end
  ...
  class HBaseInstall < HBaseCommand
    ...
    def modify_jobflow(jobflow)
      jobflow['AdditionalInfo'] ||= '{ami64: "ami-50802439", amiHvm64: "ami-5e802437", hadoopVersion: "0.20.205", hadoopConfigurationVersion: "2.0"}'
  ...
  class HBaseBackupSchedule < HBaseCommand
    ...
    attr_accessor :time_interval, :time_unit, :backup_dir, :start_time, :disable
    ...
    def validate
      super
      unless get_field(:disable, false) then
        require(:backup_dir,    "--backup_dir path must be defined")
        require(:time_interval, "--time-interval is required")
        require(:time_unit,     "--time-unit is required")
      end
    end

    def steps
      args = get_step_args("--set-scheduled-backup", get_field(:disable, false) ? "false" : "true")
      if get_field(:time_interval, nil) then
        args += ["--time-interval", get_field(:time_interval, nil)]
      end
      if get_field(:time_unit, nil) then
        args += ["--time-unit", get_field(:time_unit, nil)]
      end
      if get_field(:start_time, nil) then
        args += ["--start-time", get_field(:start_time, nil)]
      end
      step = {
  ...
  def self.add_commands(commands, opts)
    ...
    commands.parse_command(HBaseBackup,          "--hbase-backup",               "Restore HBase from S3")
    commands.parse_command(HBaseRestore,         "--hbase-restore",              "Backup HBase to S3")
    ...
    commands.parse_options(["--hbase-schedule-backup"], [
      [ OptionWithArg, "--time-unit TIME_UNIT",         
          "time units for time-interval either minutes, hours or days",                            :time_unit],
      [ OptionWithArg, "--time-interval TIME_INTERVAL", "The time between backups",                :time_interval],
      [ OptionWithArg, "--start-time START_TIME",       "The time of the first backup",            :start_time],
      [ OptionWithArg, "--disable",                     "Stop scheduled backups from running",     :disable],
    ])
